  ...
  public List<Map<String, Object>> getList(String start, String end, String company, String spjangcd) {
    ...
    String sql = """
        ...
        final_prev_amt AS (
            SELECT
                y.cltcd,
                y.yearamt + COALESCE(p.extra_sales, 0) - COALESCE(p.extra_accin, 0) AS amount
            FROM last_amt y
        ...
        SELECT
            ...
            x.accin,
            x.totalamt,
            SUM(COALESCE(x.amount, 0) + COALESCE(x.totalamt, 0) - COALESCE(x.accin, 0))
...
//    log.info("미수금 잔액 명세서 read SQL: {}", sql);
